...
Every commit on `main` must be green. PRs land as a squash or a merge commit, so a
PR's own commits need not each be green.

`CHANGELOG.md` is generated by `just changelog`.
